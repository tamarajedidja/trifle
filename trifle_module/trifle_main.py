...
def main():
    parser = argparse.ArgumentParser(description="Run full TRIFLE pipeline.")
    parser.add_argument("--level", choices=["group", "individual"], required=True, help="Run at group level (group ICA) or individual level (dual regression).")
    parser.add_argument("--input", required=True, help="Input file(s): group text file or single .nii.gz")
    ...
    parser.add_argument("--mask", type=str, help="Path to brain mask.")
    parser.add_argument("--template", type=str, choices=["smith20", "smith70"], help="Template name for dual regression if level=individual.")
    ...
    try:
        ...
        if args.level == "group":
            ...
            if args.template is None:
                raise ValueError("--template is required when --level is individual")
            spatial_output_dir = output_dir / "spatial"
            spatial_output_dir.mkdir(exist_ok=True)
            trifle_spatial.run_dual_regression(args.template, args.input, spatial_output_dir)
            S_path = spatial_output_dir / "dr_stage2_ic.nii.gz"
            T_path = spatial_output_dir / "dr_stage1_subject0000.txt"
        ...
        trifle_tvM.run_tv_mix(
            input_nii=args.input,
            spatial_map_nii=S_path,
            time_series_file=T_path,
            M_file=M_path,
            B_file=B_path,
            mask_file=args.mask,
        ...
        # Create ZIP archive of full output
