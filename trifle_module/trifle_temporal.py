...
def run_temporal_ica(input_path, output_path, model_order, seed=42):
    """
    Run temporal ICA on spatial ICA time series (melodic_mix).
    Saves M, B, and full model for the specified model order.
    """
    input_path          = Path(input_path)
    if not input_path.is_file():
        raise FileNotFoundError(f"Input time series file not found: {input_path}")
...
def main():
    ...
    parser.add_argument("--input", required=True, help="Path to the spatial ICA time series matrix (e.g., melodic_mix).")
