...
Nd                  = 70; Nk=21                 #spatial and temporal model order
...
# Fig. 7. Panel A  Group level statistics for trial-average f SMITH70
...
#TODO: add behavioural to data folder and remove local reference
    ...
    if Ndel_dict[sub_ses] > 0:
        succes_cut[sub_ses] = succes_pd[:-Ndel_dict[sub_ses]]
    else:
...
hits70_allpp                = np.zeros([14,70, 60]) #dataframes for all pp's averaged over trials
...
#TODO: add behavioural to data folder and remove local reference
    ...
    reg                     = np.zeros([3000])
    onsets_visual           = behavior[:,0]; onsets_visual_f = onsets_visual/TR
    ...
    del behavior, reg,onsets_visual, onsets_visual_f, n_del, epoch_start, epoch_ends, s, hit_start, fail_start
...
cols_to_zscore              = [ "Visual:HRF", "Visual:Shift", "Visual:Dispersion", "Visual:4", "Visual:5", "Visual:6","Motor:HRF",  "Motor:Shift",  "Motor:Dispersion",  "Motor:4",  "Motor:5",  "Motor:6"]
for col in cols_to_zscore:
    scaler                  = preprocessing.StandardScaler().fit(df[[col]])
    df[f"{col.replace(':', '')}_z"] = scaler.transform(df[[col]])
...
df2                         = pd.DataFrame({'PP': pp_id, 'Session': ses_id, 'Timepoint': tp_id, 'Visual:HRF': subses_bfs[:, 0], 'Visual:Shift': subses_bfs[:, 1], 'Visual:Dispersion': subses_bfs[:, 2], 'Visual:4': subses_bfs[:, 3], 'Visual:5': subses_bfs[:, 4], 'Visual:6': subses_bfs[:, 5], 'Motor:HRF': subses_bfs[:, 6], 'Motor:Shift': subses_bfs[:, 7], 'Motor:Dispersion': subses_bfs[:, 8], 'Motor:4': subses_bfs[:, 9], 'Motor:5': subses_bfs[:, 10], 'Motor:6': subses_bfs[:, 11], 'f_primVis_1': subses_fs[:, 17], 'f_primVis_2': subses_fs[:, 41], 'f_primVis_3': subses_fs[:, 53], 'f_primVis_4': subses_fs[:, 63], 'f_lateralVis_1': subses_fs[:, 10], 'f_lateralVis_2': subses_fs[:, 24], 'f_sm_r': subses_fs[:, 50], 'f_sm_l': subses_fs[:, 45], 'f_aud': subses_fs[:, 12], 'f_hipp': subses_fs[:, 39], 'f_dmn': subses_fs[:, 30], 'f_sp': subses_fs[:, 58], 'f_ifg': subses_fs[:, 29], 'f_pc': subses_fs[:, 21]})
cols_to_zscore              = ["Visual:HRF", "Visual:Shift", "Visual:Dispersion", "Visual:4", "Visual:5", "Visual:6","Motor:HRF", "Motor:Shift", "Motor:Dispersion", "Motor:4", "Motor:5", "Motor:6"]
for col in cols_to_zscore:
    z_col                   = col.replace(":", "") + "_z"
    scaler                  = preprocessing.StandardScaler().fit(df2[[col]])
    df2[z_col]              = scaler.transform(df2[[col]])
    
df2_1                       = df2[["PP", "Session", "Timepoint", 'f_primVis_1', 'f_primVis_2', 'f_primVis_3', 'f_primVis_4', 'f_lateralVis_1', 'f_lateralVis_2', 'f_sm_r', 'f_sm_l', 'f_aud', 'f_hipp', 'f_dmn', 'f_sp', 'f_ifg', 'f_pc', "VisualHRF_z", "VisualShift_z", "VisualDisp_z", "Visual4_z", "Visual5_z", "Visual6_z", "MotorHRF_z", "MotorShift_z", "MotorDisp_z", "Motor4_z", "Motor5_z", "Motor6_z"]]  
df2_lm                      = pd.concat((df2_1, pd.get_dummies(succes_reg, drop_first=True)), axis=1)
names_df2                   = df2_lm.columns[3:19]  
...
for net_idx, net in enumerate(names_df):
    ...
    for predictor in predictors:    # Track which network and predictor each p-value belongs to
...
alpha                       = 0.05
corrected_pval_dict         = {label: {'corrected_pval': mc_pval,'significant': mc_pval < alpha} for label, mc_pval in zip(pval_labels, mc_pvals[1])}
...
# Fig. 5 Panel B Group-Level Network Involvement 
plt.figure(figsize=(7.5, 4.8))
...
#%% Fig. 7. GROUP LEVEL FITTED NETWORK INVOLVEMENT HIT/FAIL
plt.figure(figsize=(7.5, 4.8))
