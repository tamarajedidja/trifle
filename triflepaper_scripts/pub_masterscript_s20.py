...
tvals_hitfail = np.zeros([14,20,60]); pvals_hitfail = np.zeros([14,20,60])
for pp_idx, pp in enumerate(participants):
...
# Model and contrasts setup
net = 'f_DMN'
y, X = dmatrices(f'{net} ~ VisualHRF_z + VisualShift_z + VisualDisp_z + Visual4_z + Visual5_z + Visual6_z + C(Succes, Treatment) + VisualHRF_z*C(Succes, Treatment) + VisualShift_z*C(Succes, Treatment) + VisualDisp_z*C(Succes, Treatment) + Visual4_z*C(Succes, Treatment) + Visual5_z*C(Succes, Treatment) + Visual6_z*C(Succes, Treatment)', 
                 data=df_lm, return_type='dataframe')
...
# Extracting the contrast results (?b, t, p-values)
...
# Format p-values for non-scientific notation and display
...
# Apply FDR correction for the post-hoc contrast p-values
fdr_results_contrasts = multipletests(p_values, alpha=0.05, method='fdr_bh')

# Unpack the results: significant flags and adjusted p-values
is_significant_contrasts = fdr_results_contrasts[0]  # Boolean: True for significant p-values
corrected_pvals_contrasts = fdr_results_contrasts[1]  # Adjusted p-values

# Identify which contrasts are significant
...
# Print significant contrasts and their adjusted p-values without scientific notation
...
plt.gca().set_title("Associations Dual-Regression Time Series with Visual Regressors", font = 'Futura Hv BT', fontweight="bold", fontsize=12)
g1 = sns.violinplot(x= 'Network', y = 'r-value', data= DR_corr_plot , color="0.8", inner='quartile') 
g1= sns.stripplot(x= 'Network', y = 'r-value', data= DR_corr_plot,jitter=True, palette=[colors_4[0], '#fc4f30','#fc7259', colors_4[1],'mediumseagreen',colors_4[2], '#810f7c', 'mediumorchid', 'mediumorchid', colors_4[3]]) 
g1.set(xlabel=None); g1.set(ylabel=None)
plt.xticks(rotation=20)
plt.tight_layout()
plt.xlabel("Network")
plt.ylim([-.75,1])
fig.text(0.0, 0.5, 'Fisher-$Z$ tranformed Pearson ($r$)', va='center', rotation='vertical')
plt.savefig('/home/mrstats/tamdklo/figures/P1_trifle/S20/SM_III_dr_cor_task.png', dpi=700)
plt.tight_layout()

maxval_abs_TFM_visual    = pickle.load(open('/project/3013060.04/TK_data/results_S20/maxval_abs_TFM_visual.pickle',"rb"))
maxval_abs_TFM_motor    = pickle.load(open('/project/3013060.04/TK_data/results_S20/maxval_abs_TFM_motor.pickle',"rb"))

TFM_corr_total = pd.concat([maxval_abs_TFM_visual, maxval_abs_TFM_motor], axis=1, keys=["Visual","Motor"])
TFM_corr_total_plot = TFM_corr_total.melt(var_name= 'Task Regressor', value_name= 'r-value')

## JOIN TFM VISUAL AND MOTOR
plt.figure(figsize=(7.5, 4.8))

sns.violinplot(x= 'Task Regressor', y = 'r-value', data= TFM_corr_total_plot, color="0.8", inner='quartile') 
sns.stripplot(x= 'Task Regressor', y = 'r-value', data= TFM_corr_total_plot,jitter=True, palette=[colors_4[0], colors_4[1]]) 
plt.title("Associations TFM Timeseries with Task Regressors", font = 'Futura Hv BT', fontweight="bold", fontsize=12)
plt.xlabel("Task Regressor")
...
#%%
# I.E. INDEPENDENT OF THE VARIABILITY ACROSS SUBJECTS, THE HIGHEST TFM COR IS ALWAYS HIGHER THAN DR, E.G. 
maxval_abs_DR_visual    = pickle.load(open('/project/3013060.04/TK_data/results_S20/maxval_abs_DR_visual.pickle',"rb"))
t_dr_tfm     = ss.ttest_rel(maxval_abs_DR_visual, maxval_abs_TFM_visual, alternative='less'); print(t_dr_tfm) # highest DR of the 20
